...
async function createAsset(data) {
    try {
        const result = await submitTransaction(userId, org, channel, chaincodeName, "CreateAsset", JSON.stringify(data));
...
async function updateAsset(data) {
    try {
        const result = await submitTransaction(userId, org, channel, chaincodeName, "UpdateAsset", JSON.stringify(data));
        return JSON.parse(result);
    }   catch (error) {
...
async function getAllAssets() {
    try {
        const result = await evaluateTransaction(userId, org, channel, chaincodeName, "GetAllAssets");
        return JSON.parse(result);
    }   catch (error) {
...
async function getAssetById(assetId) {
    try {
        const result = await evaluateTransaction(userId, org, channel, chaincodeName, "ReadAsset", assetId);
        return JSON.parse(result);
    }   catch (error) {
...
async function getAssetHistory(assetId) {
    try {
        const result = await evaluateTransaction(userId, org, channel, chaincodeName, "GetAssetHistory", assetId);
        return JSON.parse(result);
    }   catch (error) {
...
async function transferAsset(assetId, newOwner) {
    try {
        const result = await submitTransaction(userId, org, channel, chaincodeName, "TransferAsset", assetId, newOwner);
...
function handleServiceError(error, defaultMessage, assetId = "") {
    if (error.message.includes("does not exist")) {
        throw { status: 404, message: `Asset not found${assetId ? `: ${assetId}` : ""}` };
    }
