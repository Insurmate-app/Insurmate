...
router.put(
    ...
    passport.authenticate("jwt", { session: false }),
    [
    body("assetId").notEmpty().withMessage("Asset ID is required").isString().withMessage("Asset ID must be a string"),
    body("newOwner").notEmpty().withMessage("New owner is required").isString().withMessage("New owner must be a string")
    ],
    validate,
